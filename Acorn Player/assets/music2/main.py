...
###DEF###
def remove_end_silence(input_folder="assets/music2/"):
...
def delete_all_song_file():
    ...
    for filename in os.listdir("assets/music2/"):
        file_path = os.path.join("assets/music2/", filename)
        os.remove(file_path)
        refresh_list()
...
def shuffle():
    ...
    songs = []
    for song in os.listdir('assets/music2/'):
...
def refresh_list():
    ...
    for song in os.listdir('assets/music2/'):
        if song.endswith('.webm'):
            # Convert to mp3
            subprocess.run('ffmpeg -i "assets/music2/' + song + '" "assets/music2/' + re.sub(r'.webm', '.mp3', song) + '"', shell=True)
            # Delete the webm file
            os.remove("assets/music2/" + song)
...
def add_song_file():
    ...
    shutil.copyfile(song, 'assets/music2/' + re.sub(r'.*\/', '', song))
...
def Download(link):
    options = {
        'format': 'bestaudio',
        'outtmpl': os.path.join("assets/music2/", '%(title)s.%(ext)s'),
        'quiet': True
    }
    ...
    try:
        subprocess.run('ffmpeg -i "assets/music2/' + title + '.webm" "assets/music2/' + title + '.mp3"', shell=True)
        os.remove("assets/music2/" + title + ".webm")
